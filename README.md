...
$$P(\text{positive}),\ P(\text{neutral}),\ P(\text{negative}) \quad \text{s.t.} \sum P = 1$$
...
$$\text{gemma\_score} = P(\text{positive}) - P(\text{negative})$$
...
$$\text{ensemble\_score} = \alpha \cdot \text{finbert\_score} + (1-\alpha) \cdot \text{gemma\_score}$$

기본값: $\alpha = 0.5$ (`.env`의 `FINBERT_WEIGHT`로 조정 가능)

**합의 점수 (모델 간 신뢰도 지표)**:

$$\text{agreement\_score} = 1 - \frac{|\text{finbert\_score} - \text{gemma\_score}|}{2}$$
...
기사의 감성이 모호한 경우 상관관계 분석에서 제외합니다.

$$\text{is\_valid} = \neg \left( P(\text{neutral}) \geq 0.5 \ \lor\ |P(\text{pos}) - P(\text{neg})| < 0.15 \right)$$

---

### 4. 5분 윈도우 신뢰도 가중집계

같은 5분 구간 내 여러 기사를 하나의 감성 신호로 집계할 때, 확신도가 높은 기사에 더 큰 가중치를 부여합니다.

**신뢰도 (Confidence)**:

$$c_i = \max\left(P_i(\text{positive}),\ P_i(\text{negative})\right)$$

> 중립에 가까운 기사일수록 $c_i$가 낮아 영향이 줄어듭니다.

**윈도우 감성 점수**:

$$\text{window\_score} = \frac{\displaystyle\sum_{i} \text{score}_i \cdot c_i}{\displaystyle\sum_{i} c_i}$$

> 예시:  
> 기사 A: score=+0.8, confidence=0.9 → 기여도 0.72  
> 기사 B: score=−0.3, confidence=0.4 → 기여도 −0.12  
> window_score = (0.72 − 0.12) / (0.9 + 0.4) ≈ **+0.46**
...
$$\text{return}_{+Nm} = \frac{\text{close}_{t+N} - \text{close}_t}{\text{close}_t} \times 100\ (\%)$$
...
감성 점수와 이후 가격 수익률 사이의 선형 상관도를 측정합니다.

$$r = \frac{\displaystyle\sum_{i}(x_i - \bar{x})(y_i - \bar{y})}{\sqrt{\displaystyle\sum_{i}(x_i - \bar{x})^2} \cdot \sqrt{\displaystyle\sum_{i}(y_i - \bar{y})^2}}$$

- $x_i$: 윈도우 감성 점수, $y_i$: T+N 수익률
...
### 7. (예정) 방향 예측 적중률 (Direction Accuracy)

감성 점수의 부호로 가격 방향(상승/하락)을 예측하고 실제 방향과 비교합니다.

$$\hat{y}_i = \begin{cases} 1\ (\text{상승 예측}) & \text{ensemble\_score}_i > 0 \\ 0\ (\text{하락 예측}) & \text{otherwise} \end{cases}$$

$$y_i = \begin{cases} 1\ (\text{실제 상승}) & \text{return}_{+Nm} > 0 \\ 0\ (\text{실제 하락}) & \text{otherwise} \end{cases}$$
...
$$\alpha^* = \underset{\alpha}{\arg\max}\ \frac{1}{4}\sum_{\tau \in \{5, 15, 30, 60\}} \text{Accuracy}_\tau(\alpha)$$
